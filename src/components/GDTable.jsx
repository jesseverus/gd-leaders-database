...
// ─── Awaiting Induction: local rank extension (does not modify utils) ─────────
const AI_RANK = "Awaiting Induction";
const GD_RANKS        = RANKS.includes(AI_RANK) ? RANKS : [...RANKS, AI_RANK];
const GD_RANK_ORDER   = RANK_ORDER[AI_RANK] !== undefined ? RANK_ORDER
  : { ...RANK_ORDER, [AI_RANK]: Math.max(...Object.values(RANK_ORDER)) + 1 };
const GD_RANK_META    = { ...RANK_META,  [AI_RANK]: { bg:"#1a2535", fg:"#64748b", tier:"awaiting" } };
const GD_TIER_ROW     = { ...TIER_ROW,  awaiting: { even:"#0d131d", odd:"#0f1520" } };
...
const sorted=[...officers].filter(o=>!sq||[o.steamName,o.fullName,o.callsign].some(v=>(v||"").toLowerCase().includes(sq))).sort((a,b)=>{const rd=(GD_RANK_ORDER[a.rank]??99)-(GD_RANK_ORDER[b.rank]??99);if(rd!==0)return rd;return(daysSince(b.lastPromotionDate)??-1)-(daysSince(a.lastPromotionDate)??-1);});
...
const thTop={...thB,top:0,zIndex:5,fontSize:8,letterSpacing:"0.12em",color:"#3a5878",borderBottom:"none",background:T.nav};
...
// Simpler: use the `days` (daysSince lastPromotionDate) and compare against rank thresholds.
...
const toggleRank=(rank)=>setCollapsedRanks(prev=>{const n=new Set(prev);n.has(rank)?n.delete(rank):n.add(rank);return n;});
...
displayList.forEach(o=>{
  if(o.rank!==lastRank2){lastRank2=o.rank;ri2=0;
    ...
    const cnt=displayList.filter(x=>x.rank===o.rank).length;
    const isCollapsed=collapsedRanks.has(o.rank);
    collapsibleRows.push(
      <tr key={`g_${o.rank}`} style={{cursor:"pointer"}} onClick={()=>toggleRank(o.rank)}>
        <td colSpan={totalCols} style={{background:"#040710",padding:"5px 12px",borderBottom:`1px solid ${T.borderMid}`,borderTop:collapsibleRows.length>0?`2px solid ${T.borderMid}`:"none",position:"sticky",left:0,zIndex:3,whiteSpace:"nowrap",userSelect:"none"}}>
          <span style={{background:rm.bg,color:rm.fg,borderRadius:3,padding:"2px 10px",fontSize:9,fontWeight:800,letterSpacing:"0.12em"}}>{o.rank.toUpperCase()}</span>
          <span style={{color:T.muted,fontSize:9,marginLeft:10}}>{cnt} officer{cnt!==1?"s":""} · sorted by tenure</span>
          <span style={{color:T.hint,fontSize:9,marginLeft:8,opacity:0.6}}>{isCollapsed?"▶ collapsed":"▼"}</span>
        </td>
      </tr>
    );
  }
  if(collapsedRanks.has(o.rank))return; // skip rows for collapsed ranks
  const rm=GD_RANK_META[o.rank]||{tier:"silver"};
  const bg=GD_TIER_ROW[rm.tier||"silver"][ri2%2===0?"even":"odd"];ri2++;
