...
export function PortTrialTab({portTrials,onUpsert,onRemove,search}){
  ...
  // Filter then group by year, sorted most-recent year first, entries within each year newest first
  const { years, byYear } = useMemo(() => {
    const filtered = [...portTrials]
      .filter(t => !sq || [t.name,t.rank].some(v=>(v||'').toLowerCase().includes(sq)))
      .sort((a,b) => a.startDate > b.startDate ? -1 : 1); // newest first within group
    ...
    const years = Object.keys(byYear).sort((a,b) => Number(b)-Number(a)); // most recent year first
    return { years, byYear };
  }, [portTrials, sq]);

  // Most recent year is expanded by default, all others collapsed
  ...
  const inProgress = portTrials.filter(t=>t.outcome==='In Progress').length;
    ...
    <div style={{padding:'10px 14px',overflowY:'auto',maxHeight:'calc(100vh - 102px)'}}>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:12,flexWrap:'wrap',gap:8}}>
        <div style={{fontSize:13,color:T.hint}}>
          PORT Trial Period · {portTrials.length} entries · grouped by year · {inProgress} in progress
        </div>
      ...
      {years.length===0 && (
        <div style={{color:T.muted,textAlign:'center',padding:32,fontSize:13}}>No trials match.</div>
      )}
      ...
      {years.map(year => {
        ...
        const active_  = entries.filter(t=>t.outcome==='In Progress').length;
        ...
        return (
          <div key={year} style={{marginBottom:10}}>
            ...
            <div onClick={()=>toggleYear(year)} style={{display:'flex',alignItems:'center',gap:10,
              ...
              <span style={{color:T.accent,fontSize:11}}>{closed?'▶':'▼'}</span>
              ...
              {active_>0  && <span style={{background:'#78350f',color:'#fef9c3',borderRadius:10,padding:'1px 7px',fontSize:10,fontWeight:700}}>{active_} active</span>}
              {finished>0 && <span style={{color:'#22c55e',fontSize:10,marginLeft:2}}>{finished} finished</span>}
              {failed>0   && <span style={{color:'#ef4444',fontSize:10,marginLeft:2}}>{failed} didn't finish</span>}
            </div>
                  ...
                  return (
                    <div key={t.id} style={{
                      ...
                      <div style={{flex:'0 0 80px',fontSize:12}}>
                        ...
                        <div style={{color:T.text}}>{fmtShort(t.endDate)}</div>
                      </div>
